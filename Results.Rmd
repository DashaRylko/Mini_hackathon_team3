...
final_table <- add_footer_lines(final_table, "Footnote: N - not missing, Nmiss - missing") %>%
  theme_box() %>% 
  separate_header() %>%
...
final_table_cat <- add_footer_lines(final_table_cat, "Footnote: Percentages are based on the number of subjects in the corresponding group") %>%
  theme_box() %>% 
  separate_header() %>%
...
#anova
anova_res <- aov(nrAdvE ~ treat, data = data_types)
summary(anova_res)

#t-test для d1 - placebo и d2 - placebo
data_d1 <- data_types %>% 
  filter(treat != 'd2')
data_d2 <- data_types %>% 
  filter(treat != 'd1')

t.test(nrAdvE ~ treat, data = data_d1)
t.test(nrAdvE ~ treat, data = data_d2)

#линейная и generalized linear model
levels(data_types$treat)
data_types$treat <- relevel(data_types$treat, ref = "placebo")

summary(glm(nrAdvE ~ treat, data = data_types))
summary(lm(nrAdvE ~ treat, data = data_types))
```
```{r}
...
res <- data.frame()
for(i in c(cor_test_age,cor_test_height,cor_test_weight,cor_test_BMI)){
  res <- rbind(res,data.frame(
  Correlation = i[estimate],
  t_statistic = i$statistic,
  df = i$parameter,
  p_value = i$p.value,
  CI_lower = i$conf.int[1],
  CI_upper = i$conf.int[2]
))
}
res
```
...
rbind(table_res_country,table_res_sex)
```
