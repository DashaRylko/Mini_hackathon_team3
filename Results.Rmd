...
author: "Ирина Михайлова, Ильнара Нуриеа, Оксана Пластинина, Дарья Рылко"
date: "2025-11-02"
...
tidy_model_age <- tidy(model_treat, conf.int = TRUE) %>% 
  mutate(across(c(estimate, std.error, statistic, conf.low, conf.high, p.value), 
                ~ round(.x, 3)))
tidy_model_age %>% 
  mutate(term=case_when(term=="(Intercept)" ~ "Placebo",
                        term=="treatd1" ~ "Drug 1",
                        term=="treatd2" ~ "Drug 2"
                        ),
  ...
  set_table_properties(layout= "autofit",width=.9)
```
Наблюдается снижение количества НЯ на 35% в группе пациентов, принимавших препарат 1 по сравнению с группой пациентов, принимавших Плацебо, не обнаружено значимых различий в частоте НЯ среди пациентов, принимавших препарат 2 и принимавших Плацебо.


#### Корреляционный анализ (количество НЯ + количественные признаки)
```{r}
...
`Количество НЯ и масса тела` = data.frame(
...
```
Не наблюдается значимых ассоциаций между количественными признаками - Масса тела, Возраст, Рост, ИМТ.



#### Влияние качественных признаков на количесвто НЯ
```{r}
...
t_test_res %>%
  ...
  set_table_properties(layout = "autofit", width = .9)
```
Не наблюдается статистически значимых различий в количестве НЯ между разными странами и между мужчинами и женщинами.


```{r eval=FALSE}
data_types$treat <- relevel(data_types$treat, ref = "placebo")

model_treat <- glm(nrAdvE ~ treat*BMI, data = data_types, family = 'poisson')
